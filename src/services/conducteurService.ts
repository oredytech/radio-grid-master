...
// Service temporaire utilisant any pour éviter les erreurs TypeScript
// Les types seront mis à jour après la génération automatique des types Supabase
export const conducteurService = {
  ...
  async getWithElements(conducteurId: string): Promise<ConducteurWithElements | null> {
    ...
    const { data: elements, error: elementsError } = await supabase
      .from('conducteur_elements' as any)
      .select('*')
  ...
  async reorderElements(elements: { id: string; ordre: number }[]): Promise<void> {
    const updates = elements.map(({ id, ordre }) => 
      supabase.from('conducteur_elements' as any).update({ ordre }).eq('id', id)
    );
